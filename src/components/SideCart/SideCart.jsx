...
export default function SideCart({ isOpen, onClose }) {
  const { cart, removeFromCart, checkoutUrl, updateLineQty, updateLineVariant } = useShopifyCart();
  ...
  const vatRate = subtotalAmt > 0 && taxAmt > 0 ? Math.round((taxAmt / subtotalAmt) * 100) : null;

  return (
    <div className={classNames(styles.sideCart, { [styles.open]: isOpen })} role="dialog" aria-modal="true" aria-label="Shopping cart">
      <div className={styles.topBar}>
        <h3>Your Cart</h3>
        <button onClick={onClose} className={styles.closeBtn} aria-label="Close cart"><FaTimes/></button>
      </div>
      ...
      <div className={styles.items}>
        {lines.length === 0 ? (
          ...
          const img = merch?.image?.url || product?.featuredImage?.url || "https://via.placeholder.com/600x600?text=No+Image";
          const handle = product?.handle ? `/products/${product.handle}` : "/products";
          const unitAmt = merch?.price?.amount ?? line.cost?.amountPerQuantity?.amount;
          const unitCur = merch?.price?.currencyCode || line.cost?.amountPerQuantity?.currencyCode || subtotalCur;
          ...
          return (
            <div key={line.id} className={styles.cartItem}>
              <Link to={handle} onClick={onClose} className={styles.mediaLink}>
                <img src={img} alt={product?.title || merch?.title || "Product image"} loading="lazy" />
              </Link>
              ...
              <div className={styles.details}>
                ...
                {/* Variant selector – title usually encodes options (e.g., "Blue / 52") */}
                {variantNodes.length > 1 && (
                  <label className={styles.variantSelectWrap}>
                    ...
                    >
                      {variantNodes.map(v => (
                        <option key={v.id} value={v.id} disabled={v.availableForSale === false}>
                          {v.title} {v.availableForSale === false ? "— (Out of stock)" : ""}
                        </option>
                      ))}
                    </select>
                  </label>
                ...
                {!variantNodes.length && variantTitle && (
                  <p className={styles.variant}><strong>Variant:</strong> {variantTitle}</p>
                )}

                <div className={styles.row}>
                  <p><strong>Unit:</strong> {money(unitAmt, unitCur)}</p>
                ...
                <p><strong>Line:</strong> {money(line.cost?.totalAmount?.amount, line.cost?.totalAmount?.currencyCode || unitCur)}</p>

                <button className={styles.removeBtn} onClick={() => removeFromCart(line.id)}>
                  <FaTrash/> Remove
                </button>
              </div>
            </div>
      ...
      {lines.length > 0 && (
        <div className={styles.total}>
          <p><strong>Subtotal:</strong> {money(subtotalAmt, subtotalCur)}</p>
          <p><strong>MVA{vatRate ? ` (${vatRate}%)` : ""}:</strong> {money(taxAmt, subtotalCur)}</p>
          <p><strong>Total:</strong> {money(totalAmtRaw, totalCur)}</p>
        </div>
      )}

      {lines.length > 0 && checkoutUrl && (
        <button className={styles.checkoutBtn} onClick={() => (window.location.href = checkoutUrl)}>
          Go to Checkout
        </button>
      )}
    </div>
